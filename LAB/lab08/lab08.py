def convert_link(link):
    ...
    if link == Link.empty:
        ...
        return convert_link(link.first)
    else:
...
def duplicate_link(link, val):
    ...
    if link is Link.empty:
        ...
        if link.first == val:
            wedge = Link(link.first)
            wedge.rest = link.rest
            link.rest = wedge
            duplicate_link(wedge.rest, val)
        else:
...
def every_other(s):
    ...
    while s is not Link.empty:
        if is_odd(index):
            ...
            continue
        index += 1
        prior, s = prior.rest, s.rest

#网络版本
def every_other_recusive(s):
    if s is Link.empty or s.rest is Link.empty:
        return
    s.rest = s.rest.rest
    return every_other_recusive(s.rest)
