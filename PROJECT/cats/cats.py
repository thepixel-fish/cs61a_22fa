...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    choose = [p for p in paragraphs if select(p)]
    if k < len(choose):
        return choose[k]
    else:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def fliter(s):
        trim = split(remove_punctuation(s))
        for t in topic:
            for word in trim:
                if t == lower(word):
                    return True
...
def accuracy(typed, source):
    ...
    def walker(index):
        if index >= len(source_words):
            return 0.0
        elif index >= len(typed_words):
            return 0.0
        else:
            if typed_words[index] == source_words[index]:
                return 1 + walker(index + 1)
            else:
                return walker(index + 1)
    if len(typed_words) == 0 and len(source_words) == 0:
        return 100.0
    elif len(typed_words) == 0:
        return 0.0
    return walker(0) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    diff = lambda word: diff_function(typed_word, word, limit)
    match1 = lambda word_list:[word for word in word_list if typed_word == word] == []
    
    close = min(word_list, default=typed_word, key=diff)
    if diff(close) <= limit and match1(word_list):
        return close
    else:
        return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    count = lambda m, n: 0 if m == n else 1
    def substitute(index, times):
        if index >= len(typed) or index >= len(source):
            return abs(len(source) - len(typed))
        else:
            sub = count(typed[index], source[index])
            climb = times + sub
            if climb > limit:
                return 1
            return sub + substitute(index + 1, climb)
...
def final_diff(start, goal, limit):
    """minimum_mewtation: A diff function that takes in a string TYPED, a string SOURCE, and a number LIMIT.
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    time = lambda s: [[s[0][i + 1] - s[0][i] for i in range(len(s[0]) - 1)]] + time(s[1:]) if s else []
...
def fastest_words(match):
    ...
    for i in word_indices:
        fast += [min(player_indices, key=lambda player: time(match, player, i))] 
    best = []
    for i in player_indices:
        best += [[get_word(match, index) for index in range(len(fast)) if i == fast[index]]]
    return best
    # END PROBLEM 10
