...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
            print("DEBUG:", "First times", self.__repr__(), place)
        else:
            # BEGIN Problem 8b
            if place.ant.is_container:
                assert place.ant.can_contain(self), 'Two ants in {0}'.format(place)
                place.ant.store_ant(self)
                print("DEBUG:", "place.Contained_Ant load", self.__repr__(), place, place.ant.can_contain)
            elif self.is_container:
                assert self.can_contain(place.ant), 'Two ants in {0}'.format(place)
                self.store_ant(place.ant)
                print("DEBUG:", "Protect place.ant", place.ant.__repr__(), place)
                place.ant = self
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.doubling and self.damage:
            self.damage = self.damage * 2
        if self.is_container and self.ant_contained is not None:
            if self.ant_contained.damage and not self.ant_contained.doubling:
                self.ant_contained.damage *= 2
            self.ant_contained.doubling = True
        self.doubling = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        aim, dist = self.place, 0
        while not aim.is_hive:
            if self.lower_bound <= dist <= self.upper_bound:
                if aim.bees != []:
                    return random_bee(aim.bees)
            aim, dist = aim.entrance, dist + 1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        ant_dead = (self.health <= amount)
        super().reduce_health(amount)
        for bee in fire_place.bees[:]:
            Insect.reduce_health(bee, amount + self.damage * ant_dead)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        self.turns_to_chew = 0 #empty belly
    def action(self, gamestate):
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        ...
        if self.can_contain(ant):
            print("DEBUG:", "store_ant",ant.__repr__())
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2, *args, **kwargs):
        return super().__init__(health, *args, **kwargs)
    # END Problem 8c

# BEGIN Problem 9
class TankAnt(ContainerAnt):
    """Providing protection to the other Ants and deal 1 damage 
        to all bees in its place.
    """
    name = 'Tank'
    damage = 1
    food_cost = 6
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        fellows = self.place.exit
        while fellows is not None:
            print("DEBUG:", fellows)
            if fellows.ant is not None:
                fellows.ant.double()
            fellows = fellows.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        """Changing bee.action to slowed_act(), and change back when
            bee.slowed_turns run out."""
        # BEGIN Problem EC
        target.slowed = True
        target.slowed_turns = self.slowed_turns
        bind_method = lambda bee, fn: lambda *args: fn(bee, *args)
    
        def slowed_act(target, gamestate):
            """Action when bee been slowed."""
            if target.slowed and gamestate.time % 2 != 0:
                pass
            else:
            ...
            if target.slowed_turns == 0:
                target.slowed = False
                target.action = bind_method(target, Bee.action)
    
        target.action = bind_method(target, slowed_act)
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        battery = self.insects_shot * self.battery_reduce
        weakened = distance * self.dist_weakened
        real_damage = self.damage - battery - weakened
        print("DEBUG:", real_damage, battery)
        return (lambda: real_damage if real_damage > 0 else 0)()
        # END Problem Optional 2
