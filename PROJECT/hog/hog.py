...
def roll_dice(num_rolls, dice=six_sided):
    ...
    def One(k):
        ...
        if k == 1:
            flag = True
        return k
    for times in range(1, num_rolls + 1):
        sum += One(dice())
    if not flag:
        return sum
    else:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    opponent_score = 0
    flag = True
    diff = strategy(score, opponent_score)
    while score < goal:
        while opponent_score < goal:
            if diff != strategy(score, opponent_score):
                flag = False
            opponent_score += 1
        score += 1
        opponent_score = 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    hightest = 0
    roll_averaged = make_averaged(roll_dice, total_samples)
    for rolls in range(1, 11):
        avg = roll_averaged(rolls, dice)
        if hightest < avg:
            hightest = avg
            num_rolls = rolls
    return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    1. roll 0 whenever it would give 
    you more points on average than rolling 6.
    2. If you are in the lead, you might decide to take fewer risks.
    3. catch up with max_score_num_rolls
    4. always take the square_strategy with 0 rolls
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    roll_ave = make_averaged(roll_dice)
    tail = tail_points(opponent_score)
    shoot = max_scoring_num_rolls() #taking a shoot
    turn_4 = always_roll(4)

    if perfect_square(score + tail) or tail > 12:
        return 0
    if score < opponent_score:
        return shoot
    else:
        if roll_ave(6) < tail:
            return 0
        elif score + roll_ave(1) >= GOAL:
            return 1
        elif score + roll_ave(2) >= GOAL:
            return 2
        else:
            return turn_4(score, opponent_score)
    # END PROBLEM 12
