...
def pingpong(n):
    ...
    def switch(n, flag):
        ...
        if n % 8 == 0 or num_eights(n) != 0:
            if flag == 1:
                return switch(n - 1, 1 - flag) - 1
            else:
                return switch(n - 1, 1 - flag) + 1
        else:
            if flag == 1:
                return switch(n - 1, flag) + 1
            else:
                return switch(n - 1, flag) - 1
    return switch(n, 0)
...
def count_coins(change):
    ...
    True
    """
    def count(change, denomin):
        if change < denomin:
            denomin = next_smaller_coin(denomin)
        if next_smaller_coin(denomin) == None:
            return 1
        elif change <= 0:
            return 1
        else:
            return count(change - denomin, denomin) + count(change, next_smaller_coin(denomin))
