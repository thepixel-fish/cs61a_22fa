...
def order_least_greatest(lnk):
    if lnk.rest is Link.empty or lnk is Link.empty:
        return True
    elif lnk.label > lnk.rest.label:
        return False
    else:
        return order_least_greatest(lnk.rest)
        
def order(lnk, key=lambda x: x):
    if lnk is Link.empty:
        return True
    elif key(lnk.label) > key(lnk.rest.label):
        return False
    else: 
        return order(lnk.rest, key)
...
def merge_muta(s, t):
    ...
    if s is Link.empty:
        ...
        s.rest = merge_muta(s.rest, t)
        return s
    else:
        t.rest = merge_muta(s, t.rest)
        return t
